...
If the field is zero magnet device must be ZF.\n
    ...
    @cast_parameters_to(
         ...
         custom=cast_custom_expression, Run_Time_Mins=float, magnet_device=magnet_device_type)	
    def estimate_time(self,
            start_temperature="keep", stop_temperature="keep", step_temperature=0,
            start_field="keep", stop_field="keep", step_field=0,
            custom="None", Run_Time_Mins=30, magnet_device="N/A"):
        ...
        if (field_set_definition == SetDefinition.SCAN):
            ...
            field_pts = 1
        return (float(Run_Time_Mins) * float(temp_pts) * float(field_pts) * 60.0)
    ...
    def run(self,
        ...
        if temp_set_definition == SetDefinition.SCAN and field_set_definition == SetDefinition.SCAN:
            ...
            self.run_temp_and_field_scans(start_temperature, stop_temperature, step_temperature,
                                          start_field, stop_field, step_field, Run_Time_Mins, inst)
        elif temp_set_definition == SetDefinition.SCAN:  # Run scans for the temperature
            self.run_scans(start_temperature, stop_temperature, step_temperature, Run_Time_Mins, inst.settemp)
        elif field_set_definition == SetDefinition.SCAN:  # Run scans for the field
            self.run_scans(start_field, stop_field, step_field, Run_Time_Mins, inst.setmag)
        else:
            # If we are not doing any scans do a run with temp and field as they are
            self.check_time_and_begin_waitfor_time_end(Run_Time_Mins)

    def check_time_and_begin_waitfor_time_end(self, Run_Time_Mins):
        """
        If Run_Time_Mins are more than zero do a run and wait for the Run_Time_Mins in that run.

        Parameters:
          Run_Time_Mins (float): The time in minutes to wait for.
        """
        if Run_Time_Mins > 0:
            g.begin(quiet=True)
            g.waitfor_time(minutes=Run_Time_Mins)
            g.end(quiet=True)
    ...
    def run_temp_and_field_scans(self, start_temperature, stop_temperature, step_temperature,
        ...
        Parameters:
          ...
          Run_Time_Mins (float): The amount of millions of events to wait for in each run.
    ...
    def run_scans(self, start, stop, step, Run_Time_Mins, set_parameter_func):
        """
