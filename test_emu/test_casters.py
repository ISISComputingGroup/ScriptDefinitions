...
class TestMagnetCaster(unittest.TestCase):
    ...
    def test_magnet_devices_are_as_expected(self):
        self.assertEqual(magnet_devices_emu, {"ZF": "Active ZF", "LF": "Danfysik", "TF": "T20 Coils"})
        self.assertEqual(magnet_devices_emuloop, {"ZF": "Active ZF", "LF": "Danfysik", "TF": "T20 Coils"})
    ...
    def test_GIVEN_invalid_magnet_WHEN_cast_THEN_raise_value_error(self):
        ...
        self.assertNotIn(invalid_magnet, magnet_devices_emuloop.keys(), "This magnet should be invalid")
        # WHEN cast THEN raise value error
        assert_that(calling(magnet_device_type_emu).with_args(invalid_magnet), raises(ValueError))
        assert_that(calling(magnet_device_type_emuloop).with_args(invalid_magnet), raises(ValueError))
    ...
    def test_GIVEN_valid_magnets_upper_and_lower_WHEN_cast_THEN_returns_correct_value(self):
        ...
        for valid_magnet_input, valid_magnet in magnet_devices_emu.items():
            ...
            self.assertEqual(valid_magnet, magnet_device_type_emu(valid_magnet_input.upper()),
                             "Should return same as dict")
            self.assertEqual(valid_magnet, magnet_device_type_emu(valid_magnet_input.lower()),
                             "Should return same as dict")
            self.assertEqual(valid_magnet, magnet_device_type_emuloop(valid_magnet_input.upper()),
                             "Should return same as dict")
            self.assertEqual(valid_magnet, magnet_device_type_emuloop(valid_magnet_input.lower()),
                             "Should return same as dict")
    ...
    def test_GIVEN_na_magnet_mix_of_case_WHEN_cast_THEN_NA_returned(self):
        ...
        self.assertEqual(magnet_device_type_emu(magnet), "N/A", "Should have uppercased and returned N/A")
        self.assertEqual(magnet_device_type_emuloop(magnet), "N/A", "Should have uppercased and returned N/A")
...
class TestCustomExpressionCaster(unittest.TestCase):
    ...
    def test_GIVEN_empty_expression_WHEN_cast_THEN_none_is_returned(self):
        self.assertEqual(cast_custom_expression_emuloop(""), "None",
